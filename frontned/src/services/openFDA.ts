import type { DrugResult, OpenFdaApiResponse } from '../types';
 ...
 * @param {'brand' | 'inn'} searchType - Tip pretrage: 'brand' za naziv brenda, 'inn' za aktivni sastojak.
 * @returns {Promise<DrugResult[]>} - Niz objekata koji sadrže informacije o leku.
 */
export const searchDrugs = async (searchTerm: string, searchType: 'brand' | 'inn' = 'brand'): Promise<DrugResult[]> => {
  if (!searchTerm) {
    return [];
  }
